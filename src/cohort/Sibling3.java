 ...
 *
 */
public class Sibling3 extends Sibling {
	...
	private static void setEarliestDOBSibling(LocalDate earliestDOBSibling) {
		Sibling3.earliestDOBSibling = earliestDOBSibling;
	}
	...
	public Sibling3(VeteranCohort veteranCohort) throws Exception {
		...
		this.setUniformdistribution(
				new UniformDistribution(Sibling3.getMintogenerateerrorval(), Sibling3.getMaxtogenerateerrorval()));
		Sibling3.setErrorResponse(this.getUtilities().randBetween(Sibling3.getMintogenerateerrorval(),
				Sibling3.getMaxtogenerateerrorval()));
	...
	public Date simulateDateOfBirth() {
		try {
			Date simulatedDOB = new Date(ThreadLocalRandom.current().nextLong(
					Date.from(Sibling3.getEarliestDOBSibling().atStartOfDay(ZoneId.systemDefault()).toInstant())
							.getTime(),
					Date.from(Sibling3.getLatestDOBSibling().atStartOfDay(ZoneId.systemDefault()).toInstant())
							.getTime()));
			this.setBirthDateSimulated(simulatedDOB);
			System.out.println("Sibling 3 DOB " + this.getBirthDateSimulated().toString());
