...
public class Sibling3 extends Sibling {
	...
	private static void setErrorResponse(int errorResponse) {
		try {
			Sibling3.errorResponse = errorResponse;
		} catch (Exception e) {
	...
	public Sibling3(Cohort cohort) throws Exception {
		...
		this.setSiblingexistssimulatedresponse(Sibling3.getErrorResponse());
	}
	...
	public Date simulateDateOfBirth() {
		try {
			Date simulatedDOB = Date.from(Instant.ofEpochSecond(ThreadLocalRandom.current()
					.nextLong(Sibling3.getEarliestdob().getTime(), Sibling3.getLatestdob().getTime())));
