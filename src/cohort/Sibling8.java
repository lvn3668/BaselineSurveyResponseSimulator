 ...
 *
 */
public class Sibling8 extends Sibling {
	...
	private static void setEarliestDOBSibling(LocalDate earliestDOBSibling) {
		Sibling8.earliestDOBSibling = earliestDOBSibling;
	}
	...
	private static void setLatestDOBSibling(LocalDate latestDOBSibling) {
		Sibling8.latestDOBSibling = latestDOBSibling;
	}

	public Sibling8(Cohort cohort) throws Exception {
		...
		if (cohort.getMultipleBirthsSimulatedResponse() == 1) {
			...
			Sibling8.setEarliestDOBSibling(cohort.getEarliestDOBForSimulation());
			Sibling8.setLatestDOBSibling(cohort.getLatestDOBForSimulation());
		} else {
	...
	public Date simulateDateOfBirth() {
		try {
			Date simulatedDOB = new Date(ThreadLocalRandom.current().nextLong(
					Date.from(Sibling8.getEarliestDOBSibling().atStartOfDay(ZoneId.systemDefault()).toInstant())
							.getTime(),
