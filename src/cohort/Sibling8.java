...
public class Sibling8 extends Sibling {
	...
	private static void setErrorResponse(int errorResponse) {
		try {
			Sibling8.errorResponse = errorResponse;
		} catch (Exception e) {
	...
	public Sibling8(Cohort cohort) throws Exception {
		...
		this.setSiblingexistssimulatedresponse(Sibling8.getErrorResponse());
	}
	...
	public Date simulateDateOfBirth() {
		try {
			Date simulatedDOB = Date.from(Instant.ofEpochSecond(ThreadLocalRandom.current()
					.nextLong(Sibling8.getEarliestdob().getTime(), Sibling8.getLatestdob().getTime())));
