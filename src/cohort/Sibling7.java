...
public class Sibling7 extends Sibling {
	...
	private static void setErrorResponse(int errorResponse) {
		try {
			Sibling7.errorResponse = errorResponse;
		} catch (Exception e) {
	...
	public Sibling7(Cohort cohort) throws Exception {
		...
		this.setSiblingexistssimulatedresponse(Sibling7.getErrorResponse());
	}
	...
	public Date simulateDateOfBirth() {
		try {
			Date simulatedDOB = Date.from(Instant.ofEpochSecond(ThreadLocalRandom.current()
					.nextLong(Sibling7.getEarliestdob().getTime(), Sibling7.getLatestdob().getTime())));
