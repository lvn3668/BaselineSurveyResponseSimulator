...
public class Sibling2 extends Sibling {
	...
	private static void setErrorResponse(int errorResponse) {
		try {
			Sibling2.errorResponse = errorResponse;
		} catch (Exception e) {
	...
	public Sibling2(Cohort cohort) throws Exception {
		...
		this.setSiblingexistssimulatedresponse(Sibling2.getErrorResponse());
	}
	...
	public Date simulateDateOfBirth() {
		try {
			Date simulatedDOB = Date.from(Instant.ofEpochSecond(ThreadLocalRandom.current()
					.nextLong(Sibling2.getEarliestdob().getTime(), Sibling2.getLatestdob().getTime())));
