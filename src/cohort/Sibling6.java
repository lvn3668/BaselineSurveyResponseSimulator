...
public class Sibling6 extends Sibling {
	...
	public Sibling6(Cohort cohort) throws Exception {
		...
		if (cohort.getMultipleBirthsSimulatedResponse() == 1) {
			...
			Sibling6.setEarliestDOBSibling(cohort.getEarliestDOBForSimulation());
			Sibling6.setLatestDOBSibling(cohort.getLatestDOBForSimulation());
		} else {
			Sibling6.setEarliestDOBSibling(LocalDate
	...
	public Date simulateDateOfBirth() {
		try {
			...
			System.out.println("Sibling 6 DOB " + this.getBirthDateSimulated().toString());
